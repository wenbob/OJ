...
## 同日增量发布：专项提醒、老师学生权限与题目分页

### 上线范围

- 学生首页增加未完成专项练习强制提醒；确认后按学生和任务版本静默 60 分钟，任务新增或修改后重新提醒。
- 专项练习默认标题改为“课后练习”。
- 老师新增学生时由服务端固定初始密码 `12345678`，只能切换学生 AI 权限或把密码重置为固定初始密码；不能修改用户名、角色、自定义头衔或删除学生。
- 老师端导航首页改为精确匹配，子页面只选中当前栏目。
- 学生题库、管理员与老师题目练习、管理员题目管理统一默认每页 50 道；提交记录等非题目列表仍为每页 20 条。
- 本次功能提交为 `6ad027e913dd6cbf606cd224186d504204c487de`，没有新增依赖或数据库迁移。

### 本地验证与构建

- `npx prisma validate`、`npx tsc --noEmit`、`npm run lint` 和 `git diff --check` 全部通过。
- `npm run test -- --maxWorkers=1`：74 个测试文件、390 项测试通过。
- 在隔离的 WSL Ubuntu 24.04 环境使用 Node.js 20.20.2、OpenSSL 3.0.13、单 worker 和 1GB Node 堆上限完成生产 standalone 构建；临时 SQLite 数据库完整应用 11 个迁移。
- 发布包大小为 50,146,515 字节，SHA-256 为 `0e68a0d3bb9695a7bd7ed8b82858723d14672b2422752f3b654112b81bbc9c28`，共 3,456 个归档条目。
- 发布包不含 `.env`、数据库、备份、根 `node_modules`、`.next/cache`、私钥或嵌套压缩包；standalone 已包含静态资源、`public` 和 `libquery_engine-debian-openssl-3.0.x.so.node`。

### 生产发布与回滚点

- 发布前 PM2 与内外健康检查正常，数据库 `quick_check` 为 `ok`，有效考试窗口和全部进行中考试均为 0。
- 生产数据库备份：`/www/backups/prod-20260725-161313-before-6ad027e.db`，备份完整性为 `ok`。
- 旧版本回滚目录：`/www/oj-old-20260725-161313`。
- 生产与本地锁文件仅有 CRLF/LF 换行差异，归一化后内容一致；服务器复用现有根 `node_modules`，未执行 `npm ci`、Next.js 构建、`seed` 或数据库初始化。
- 停止 PM2 后完成 SQLite WAL checkpoint、备份、目录切换和 `npm run db:deploy`；11 个迁移均已应用且没有待执行迁移。

### 线上验收与洁癖收尾

- PM2 `oj` 在线且只监听 `127.0.0.1:3000`；内网和公网 `/api/health`、登录页与实际 CSS 静态资源均返回 200，CSS 文件为 68,053 字节。
- 登录页 CSP、`X-Frame-Options` 和 `X-Content-Type-Options` 均存在；未登录访问 `/teacher` 和 `/admin` 均以 307 跳转登录页。
- 数据库 `quick_check` 为 `ok`、外键异常为 0；发布前后均为用户 27、题目 1,330、提交 1,481、考试 0、考试记录 0。
- 使用临时签名的真实管理员会话调用正式 `/api/problems/732/run`，Docker Judge 返回 `completed` 和 `OJ_SMOKE`；试运行前后 `Submission` 均为 1,481 条。
- PM2 错误日志最后修改时间为 13:56，早于 16:13 的本次切换；本次发布没有新增错误日志。
- 线上验收后删除服务器发布包、临时脚本和验证文件，确认 `/www/oj-new` 与失败目录不存在，清理后健康检查仍为 200；数据库备份和旧版本回滚目录继续保留。
- 本机构建包与临时日志已移入回收站；未清理其它历史备份、其它站点、股票系统或 Docker 全局资源。
