...
const getStudentHomeAssignments = cache(async (userId: number) => {
  ...
  const reminderItems: PendingAssignmentReminderItem[] = pendingAssignments.map(
    (assignment) => ({
      completedCount: assignment.problems.filter((problem) => problem.completedAt)
        .length,
  ...
  return (
    <>
      <StudentAssignmentReminderModal
        assignments={assignmentData.reminderItems}
        studentId={user.id}
      />
...
async function StudentRankProgress({ userId }: { userId: number }) {
  ...
  if (currentRanking && rankProgress) {
    return (
      <RankProgressPanel
...
async function StudentRankStat({ userId }: { userId: number }) {
  ...
  return (
    <StatItem
      icon={<Trophy size={19} />}
      label="当前天梯排名"
      value={currentRanking ? `#${currentRanking.rank}` : "待上榜"}
    />
  );
}

function StudentAssignmentOverview({
  assignmentData,
}: {
  assignmentData: Awaited<ReturnType<typeof getStudentHomeAssignments>>;
}) {
  const { recentAssignment } = assignmentData;
        ...
        <div className="grid md:grid-cols-[1fr_auto]">
          <div className="p-5 md:p-6">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-ink-600">
              {recentAssignment
                ? `${recentAssignment.title} · 已完成 ${assignmentData.recentAssignmentCompleted}/${recentAssignment.problems.length} 题`
                : "目前没有专项任务，继续保持日常训练。"}
            </p>
          </div>
          <Link
            className="arena-link-card flex min-w-64 items-center justify-between gap-4 border-t border-ink-950/10 px-5 py-4 md:border-l md:border-t-0"
            href="/student/assignments"
          >
            <span>
              <span className="data-number block text-3xl font-black text-steel">
                {assignmentData.pendingCount}
              </span>
  ...
  return (
    <>
      <CompactLink
        href="/student/assignments"
        label="专项练习"
        meta={`${assignmentPendingCount} 份待完成`}
      />
      <CompactLink
        href="/student/review"
        label="错题本与薄弱知识点"
      ...
      <CompactLink
        ...
        label="日常提交记录"
        meta={`${dailySubmissionCount} 次提交`}
      />
      <CompactLink
        href="/student/exam-submissions"
        label="考试提交记录"
        meta={`${examSubmissionCount} 次提交`}
      />
      <CompactLink
