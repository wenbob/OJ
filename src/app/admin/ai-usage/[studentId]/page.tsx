...
const adminNav = [
  { href: "/admin", label: "后台首页" }, { href: "/admin/practice", label: "题目练习" },
  { href: "/admin/problems", label: "题目管理" }, { href: "/admin/exams", label: "模拟考试" },
  { href: "/admin/users", label: "用户管理" }, { href: "/admin/submissions", label: "日常提交" },
...
export default async function AdminAiUsageStudentPage({ params, searchParams }: PageProps) {
  ...
  return (
    <AppShell nav={adminNav} title="管理员端" user={user}>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-2"><Link className="btn btn-secondary" href={`/admin/learning/${studentId}`}><BookOpen size={16} />查看学情</Link><Link className="btn btn-secondary" href="/admin/ai-usage"><ArrowLeft size={16} />返回总览</Link></div>
      </div>

      <section className="surface grid overflow-hidden bg-white/50 sm:grid-cols-2 xl:grid-cols-6">
        <Stat label="AI 权限" value={detail.student.aiAccessEnabled ? "已开通" : "未开通"} />
      ...
      <section className="surface mt-7 overflow-hidden">
        ...
        {detail.items.length === 0 ? <div className="p-10 text-center text-sm font-semibold text-ink-600">这个时间范围内没有 AI 使用记录。</div> : <div className="divide-y divide-ink-950/10">{detail.items.map((turn, index) => {
          ...
          return <Fragment key={turn.id}>{showConversationHeader ? <div className="bg-ink-950/[0.04] px-5 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black text-ink-950">{turn.conversation.problemTitle}</p><span className="text-xs font-bold text-ink-600">{turn.conversation.scope === "exam" ? `考试 · ${turn.conversation.examTitle || "历史考试"}` : "日常练习"}</span></div></div> : null}<article className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex flex-wrap gap-2"><Badge label={aiUsageModeLabels[turn.mode] || turn.mode} tone="blue" /><Badge label={aiUsageStatusLabels[turn.status] || turn.status} tone={turn.status === "success" || turn.status === "cached" ? "green" : turn.status === "pending" ? "blue" : "red"} /></div><time className="text-xs font-bold text-ink-500">{formatDateTime(turn.createdAt)}</time></div><div className="mt-4 border-l-4 border-steel bg-steel/[0.06] p-4"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-steel">学生提问</p><p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-6 text-ink-900">{turn.userContent}</p></div><details className="mt-3 border border-ink-950/10 bg-white"><summary className="cursor-pointer px-4 py-3 text-sm font-black text-ink-800">{turn.assistantContent ? "展开 AI 回复" : "查看失败原因"}</summary><div className="border-t border-ink-950/10 p-4"><p className="whitespace-pre-wrap text-sm font-semibold leading-6 text-ink-800">{turn.assistantContent || turn.errorMessage || "尚未取得最终回复。"}</p></div></details><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-[11px] font-bold text-ink-500"><span><Clock3 className="mr-1 inline" size={13} />{turn.latencyMs === null ? "耗时未知" : `${(turn.latencyMs / 1000).toFixed(1)} 秒`}</span><span><Database className="mr-1 inline" size={13} />模型调用 {turn.providerCallCount} 次</span><span><BrainCircuit className="mr-1 inline" size={13} />Token {turn.totalTokens ?? "未返回"}</span>{turn.model ? <span>模型 {turn.model}</span> : null}</div><div className="mt-4 flex flex-wrap gap-2">{turn.conversation.problemId ? <Link className="btn btn-secondary px-3 py-2" href={`/admin/practice/problems/${turn.conversation.problemId}`}><BookOpen size={14} />查看题目</Link> : null}{turn.conversation.examId ? <Link className="btn btn-secondary px-3 py-2" href={`/admin/exams/${turn.conversation.examId}/records`}><GraduationCap size={14} />考试记录</Link> : null}</div></article></Fragment>;
        })}</div>}
        <Pagination basePath={`/admin/ai-usage/${studentId}`} page={detail.page} pageSize={detail.pageSize} searchParams={rawParams} total={detail.total} totalPages={detail.totalPages} />
      </section>
    </AppShell>
  );
}
