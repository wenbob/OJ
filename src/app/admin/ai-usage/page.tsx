                ...
                {dashboard.rows.map((row) => (
                  <tr className="hover:bg-steel/[0.04]" key={row.student.id}>
                    ...
                    <td className="px-5 py-4 text-right">
                      <Link className="btn btn-secondary px-3 py-2" href={studentDetailHref(row.student.id, rawParams)}>查看对话</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function studentDetailHref(studentId: number, values: Record<string, string | string[] | undefined>) { const params = toUrlSearchParams(values); params.delete("studentId"); params.delete("q"); const query = params.toString(); return `/admin/ai-usage/${studentId}${query ? `?${query}` : ""}`; }
