...
export default async function AdminStudentLearningPage({ params, searchParams }: PageProps) {
  ...
  return (
    <AppShell nav={adminNav} title="管理员端" user={user}>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <Link className="inline-flex items-center gap-2 text-sm font-black text-steel" href={`/admin/learning?window=${window}`}>
          <ArrowLeft size={16} /> 返回学情看板
        </Link>
        <div className="flex gap-2">
          {(["7d", "30d", "all"] as const).map((value) => (
            <Link className={`btn ${window === value ? "btn-primary" : "btn-secondary"}`} href={`/admin/learning/${studentId}?window=${value}`} key={value}>
      ...
      <div className="mt-6">
        <AssignmentHistory assignments={detail.assignments.map((assignment) => ({
