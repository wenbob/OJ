...
describe("GET /api/exam-submissions/my visibility", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.findMany.mockResolvedValue([
      {
        code: "int main() {}",
        errorMessage: "HIDDEN_STDERR_EXAM_LIST",
        exam: { id: 5, title: "期中考试" },
        id: 10,
        language: "C++17",
  ...
  it("redacts the top-level runtime error in both response aliases", async () => {
    const response = await GET(
      new NextRequest("http://oj.local/api/exam-submissions/my?page=1"),
    );
    ...
    expect(JSON.stringify(body)).not.toContain("HIDDEN_STDERR_EXAM_LIST");
  });
});
