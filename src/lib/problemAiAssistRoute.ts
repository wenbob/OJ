...
import {
  aiProgrammingPromptSettingKeys,
  boolSetting,
  getSetting,
} from "@/lib/settings";
...
export async function handleProblemAssist(
  request: NextRequest,
  options: {
    audience: "staff" | "student";
    requiredProblemId?: number;
  },
) {
  ...
  if (
    options.requiredProblemId !== undefined &&
    problemId !== options.requiredProblemId
  ) {
  ...
  } else if (!boolSetting(await getSetting("aiStaffProgrammingAssistEnabled"))) {
    return NextResponse.json(
      { error: "后台编程题 AI 助手尚未开启" },
      { status: 403 },
    );
  }
  ...
  if (examId === null) {
    const enabled =
      options.audience === "staff" ||
      boolSetting(await getSetting("aiPracticeEnabled"));
    if (!enabled) {
    ...
    mode === "overview"
      ...
      : await prisma.submission.findFirst({
          where: {
            userId: auth.user.id,
            problemId,
            ...(options.audience === "staff" || examId === null
  ...
  const configuredCooldownSeconds =
    (await getAiCooldownSeconds("programming", auth.user.role)) ??
    (auth.user.role === "student" ? 20 : 30);
  let release = () => {};
