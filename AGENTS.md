...
- 管理员只在系统设置中保存非敏感的服务商、Base URL、模型和思考模式；学生助手、教师学情摘要与后台选择判断解析统一跟随，学生端不得出现调整入口。
...
- 学生 AI 助手只对编程题开放，并采用个人权限加日常/考试总开关的双重校验；服务端统一执行 20 秒冷却，学生选择判断题不得显示 AI 入口。管理员和老师的选择判断校题解析属于独立功能，不得复用或放宽学生助手权限。
...
- 后台选择判断解析的非缓存生成按账号执行 30 秒冷却，并与现有 AI 请求共享全局并发限制；同一小题生成期间必须加锁。学生和未登录请求必须拒绝。
