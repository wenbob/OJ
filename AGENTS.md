...
- 学生和老师只能提交、读取自己的反馈；只有管理员可读全部、更新状态、追加回复。不得复用允许老师访问的通用后台鉴权，详情和截图越权统一 404。
- 截图必须限制实际请求字节、真实格式与解码像素，经压缩后存入数据库；不得放入 `public`、公开缓存或图片优化代理。附件 GET 每次鉴权，所有反馈 API 使用 `private, no-store`。
- 反馈与附件原子保存，回复追加且与状态更新同事务；账号删除只清空身份关联，保留用户名/角色快照及历史内容。日志不得记录反馈正文和图片。
