...
- 管理员可以管理全部角色；老师只能枚举和创建学生、调整学生 AI 权限，以及把学生密码重置为固定初始值 `12345678`。老师禁止修改学生用户名、角色或自定义头衔，禁止删除学生，也禁止读取和维护老师或管理员。
...
- 管理员只在系统设置中保存两套非敏感的服务商、Base URL、模型和思考模式：编程题配置用于学生助手与教师学情摘要，选择判断配置用于后台客观题解析；学生端和老师端不得出现调整入口。
...
- 学生 AI 助手只对编程题开放，并采用个人权限加日常/考试总开关的双重校验；学生选择判断题不得显示 AI 入口。管理员和老师的选择判断校题解析属于独立功能，不得复用或放宽学生助手权限。
...
- AI 触发间隔由管理员按学生编程、老师/管理员学情摘要、老师/管理员选择判断解析分别配置 5–600 秒；冷却桶按账号与题型配置隔离，编程和客观题互不影响。只有真正开始上游调用时才记录时间，有效缓存和幂等重放绕过冷却但仍遵守原有使用统计；上游调用后的超时或失败仍计时。同一逻辑请求的自动重试只记录一次。
...
- 后台选择判断 AI 解析由 `aiObjectiveExplanationEnabled` 独立控制且默认关闭，只允许管理员和老师使用；浏览器只能提交题目 ID 与小题序号，题干、选项和标准答案必须由服务端读取。
- `ObjectiveAiExplanation` 按 `problemId + itemIndex` 共享缓存结构化解析。题面、选项、数据库答案或 AI 配置指纹变化时缓存失效；老师可以生成缺失解析，只有管理员能强制重新生成，生成失败不得覆盖已有解析。
- 数据库标准答案是唯一判定依据，`isCorrect` 必须由服务端生成，不能信任模型判断。解析缓存不得保存 API Key、完整 Prompt、内部推理或上游原始响应，也不得写入学生 AI 使用统计。
- 后台选择判断解析的非缓存生成按管理员配置的老师/管理员角色间隔执行，并与编程 AI 请求共享全局并发限制；同一小题生成期间必须加锁。有效缓存不消耗间隔，学生和未登录请求必须拒绝。
- 桌面端选择判断校题右栏固定为上方 AI 解析 70%、下方答案输入 30%，两个区域独立滚动；移动端按题目、解析、答案输入自然排列。功能关闭或编程题页面必须保持原布局。
