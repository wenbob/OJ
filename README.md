...
这是一个基于 Next.js 的 C++ 在线 OJ 练习平台 Demo。当前版本已经覆盖编程题与选择判断题两种独立题型、日常刷题、模拟考试、考试防退出、学生单设备登录、历史 AC 题目标记、运行样例与自定义输入、代码提交、自动评测、AI 思路提示、提交详情、错题本与薄弱知识点、教师学情看板与专项练习、学生头衔和天梯榜、独立老师端、管理员题目管理、分级用户管理、系统设置、Markdown 批量导入、Docker Judge 和基础提交队列等核心流程。
...
- 开启后，管理员和老师可在题目练习详情及模拟考试练习中点击每道小题标准答案旁的“AI 解析”。学生端不会显示入口、解析或标准答案。
...
- 有效解析按题目小题共享。题面、选项、答案或服务商配置变化后缓存自动失效；老师可生成缺失解析，只有管理员可以确认后强制重新生成。
...
- AI 服务商 `aiProvider`（DeepSeek、豆包/火山方舟或自定义 OpenAI-compatible）
- AI Base URL `aiBaseUrl`（官方服务固定，自定义服务可填写）
- AI 模型 `aiModel`、思考模式 `aiThinkingMode` 和自定义思考协议 `aiCustomThinkingProtocol`
...
- 模型配置只允许管理员在系统设置中调整，学生端和教师学情页只读跟随。保存后会同时切换学生助手、教师摘要和后台选择判断解析；旧模型的学生提示、教师摘要及选择判断解析缓存不会复用。
